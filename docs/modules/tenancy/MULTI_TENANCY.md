...
| **MT-4** | tenant-scoped 테이블에 `tenant_id` 부착 (products/companies/performances/settlements/sales_quotas) | 점진 적용 |
| **MT-5** | Policy 테넌트 조건 + super_admin 우회/선택 | 권한 |
| **MT-6** | super_admin 제약사 관리 화면(CRUD) + admin 의 소속 sales 관리 범위 조정 | UI |
...
| MT-3 | ⚪ 대기 | `TenantScope` + `ResolveTenant` 미들웨어 (격리 엔진) |
| MT-4~8 | ⚪ 대기 | MT-4 에서 도메인 테이블 tenant_id 일괄 부착·백필 |
...
**문서 버전**: 1.2
**작성일**: 2026-05-29
**최종 갱신**: 2026-05-29 (MT-2 1부 완료 — 기본 제약사 시드 + users 백필. 5/5 신규, 전체 286/286 PASS)
**상태**: 🟡 구현중 — MT-1·MT-2(1부) 완료, 다음 **MT-3(`TenantScope` + `ResolveTenant`)**. 도메인 테이블 tenant_id 는 MT-4.
