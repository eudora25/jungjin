...
- **D-2 계정 생성 권한**: ✅ **A. 위임형**. super_admin → 제약사(tenant) + 제약사 admin 생성. 제약사 admin → **자사 sales 직접 생성·관리**(자기 테넌트 한정). super_admin 끼리 생성은 시드/별도 승격(D 후속). → `/users` 화면은 admin 에게 **자사 sales 만** 노출.
  - **2026-05-29 보강**: 제약사 등록(`platform.tenants.store`) 시 **초기 관리자(pharma) 계정을 함께 입력받아 1트랜잭션으로 동시 생성**(tenant + users.role=pharma). 기존 상세 화면의 `storeAdmin`(추가 관리자 계정 생성)은 그대로 유지. → 제약사는 항상 운영 가능한 관리자 계정을 갖고 시작.
