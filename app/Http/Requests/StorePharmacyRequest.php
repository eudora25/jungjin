<?php

namespace App\Http\Requests;

use App\Http\Requests\Concerns\NormalizesBusinessNumber;
use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class StorePharmacyRequest extends FormRequest
{
    use NormalizesBusinessNumber;

    public function authorize(): bool
    {
        return $this->user()?->isPlatform() ?? false; // MT-8: 공유 마스터 직접 쓰기는 platform 전용
    }

    protected function prepareForValidation(): void
    {
        $this->normalizeBusinessNumber();
    }

    public function rules(): array
    {
        return [
            'pharmacy_code' => ['nullable', 'string', 'max:50', Rule::unique('pharmacies', 'pharmacy_code')->whereNull('deleted_at')],
            'pharmacy_name' => ['required', 'string', 'max:255'],
            'business_registration_number' => ['nullable', 'string', 'max:20', Rule::unique('pharmacies', 'business_registration_number')->whereNull('deleted_at')],
            'representative_name' => ['nullable', 'string', 'max:100'],
            'postcode' => ['nullable', 'string', 'max:10'],
            'address' => ['nullable', 'string'],
            'landline_phone' => ['nullable', 'string', 'max:20'],
            'mobile_phone' => ['nullable', 'string', 'max:20'],
            'contact_person_name' => ['nullable', 'string', 'max:100'],
            'contact_phone' => ['nullable', 'string', 'max:20'],
            'email' => ['nullable', 'email', 'max:255'],
            'remarks' => ['nullable', 'string'],
            'status' => ['sometimes', Rule::in(['active', 'inactive'])],
        ];
    }

    public function attributes(): array
    {
        return [
            'pharmacy_code' => '약국 코드',
            'pharmacy_name' => '약국명',
            'business_registration_number' => '사업자등록번호',
            'representative_name' => '대표자명',
            'postcode' => '우편번호',
            'address' => '주소',
            'landline_phone' => '대표 전화',
            'mobile_phone' => '휴대폰',
            'contact_person_name' => '담당자명',
            'contact_phone' => '담당자 연락처',
            'email' => '이메일',
            'remarks' => '비고',
            'status' => '상태',
        ];
    }
}
