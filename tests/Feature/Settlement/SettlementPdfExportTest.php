...
test('관리자는 정산 PDF 를 다운로드할 수 있다', function () {
    ...
    $res = $this->actingAs($admin)->get(route('settlements.export.pdf', $settlement));
    $res->assertOk();
    $res->assertHeader('content-type', 'application/pdf');
    expect($res->streamedContent())->toStartWith('%PDF');
});

test('영업사원은 정산 PDF 를 다운로드할 수 없다', function () {
    ...
    $this->actingAs($sales)
        ->get(route('settlements.export.pdf', $settlement))
        ->assertForbidden();
});
