...
test('병의원 등록 시 현재 사업자번호가 이력에 시드된다', function () {
    ...
    expect($current)->not->toBeNull()
        ->and($current->business_registration_number)->toBe('111-11-11111')
...
test('사업자번호 변경 시 과거 번호는 마감되고 새 번호가 현재가 된다', function () {
    ...
    // 본 컬럼은 새 번호로 갱신
    expect($hospital->business_registration_number)->toBe('234-56-78901');

    // 과거 번호는 마감(is_current=false, valid_to 설정)
    $old = $hospital->numberHistories()->where('business_registration_number', '123-45-67890')->first();
    expect($old->is_current)->toBeFalse()
        ->and($old->valid_to->toDateString())->toBe('2026-05-31');

    // 새 번호가 현재
    $new = $hospital->currentNumberHistory();
    expect($new->business_registration_number)->toBe('234-56-78901')
...
test('과거(폐업) 번호로 검색하면 현재 사용 중인 병의원이 목록에 나온다', function () {
    ...
    // 옛 번호 123 으로 검색 → 현재 번호 234 를 쓰는 재등록의원이 나와야 함
    $this->actingAs($admin)
        ...
        ->assertInertia(fn ($page) => $page
            ...
            ->where('hospitals.data.0.business_registration_number', '234-56-78901')
            ->where('hospitals.data.0.matched_old_numbers.0', '123-45-67890')
        );
});
...
test('약국도 사업자번호 변경·과거번호 검색이 동작한다', function () {
    ...
    expect($pharmacy->fresh()->business_registration_number)->toBe('432-65-10987');
    ...
    $this->actingAs($admin)
        ...
        ->assertInertia(fn ($page) => $page
            ...
            ->where('pharmacies.data.0.business_registration_number', '432-65-10987')
        );
});
