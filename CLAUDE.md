...
- **`docs/modules/tenancy/MULTI_TENANCY.md`**: 멀티테넌시(GAP-10, XL) — 제약사 테넌트 + `super_admin`/`admin`/`sales` 역할 계층 + 데이터 격리 (**구현중**: MT-1·MT-2(1부) 완료, 다음 MT-3)
...
- Laravel 11 + Breeze(Inertia/Vue/TS/SSR) + PrimeVue(Sakai) 기반으로 **핵심 기능 구현 완료**:
  ...
  - GAP-10 MT-2(1부): 기본 제약사 시드 + users 백필 — 🟢 **완료** (도메인 테이블 tenant_id 는 MT-4 로 연기)
- 테스트: `./vendor/bin/sail test` 기준 **286개 전체 통과** (2026-05-29)
...
- **Now**: ~~MT-1·MT-2(1부)~~ 🟢 완료 → **MT-3(TenantScope·ResolveTenant 격리 엔진)** → MT-4(도메인 테이블 tenant_id 부착·백필)
...
## 작업 시작 시
1. `docs/planning/ROADMAP.md` §3 확인 (현재: **경로 B** — GAP-10 **MT-3**부터, `docs/modules/tenancy/MULTI_TENANCY.md`)
